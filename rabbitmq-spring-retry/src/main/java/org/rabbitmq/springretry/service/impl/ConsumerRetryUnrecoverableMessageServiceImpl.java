 ...
 */
@Service
public class ConsumerRetryUnrecoverableMessageServiceImpl implements ConsumerMessageService {

    private static final Logger LOG = LoggerFactory.getLogger(ConsumerRetryUnrecoverableMessageServiceImpl.class);

    @Autowired
    private ConsumerRetryService consumerRetryService;

    /**
     * Consume message RabbitMQ with Unrecoverable Retry configuration.
     *
     * @param message - Message
     */
    @Override
    @RabbitListener(queues = {QueueRetryUnrecoverableConfig.QUEUE_NAME})
    public void sendMessage(@Payload final Message message) {
        this.consumerRetryService.consumerWithRetryUnRecoverable(message);
