...
pub fn main(init: std.process.Init) !void {
    ...
    const message = try messageFromArgs(allocator, args, 1);
    ...
    _ = try ch.quorumQueue("task_queue");

    try ch.publishToQueue("task_queue", message, bunny.BasicProperties.persistent);
    try out.print(" [x] Sent {s}\n", .{message});
}
